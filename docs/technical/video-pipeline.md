...
| | B 站 | 抖音 |
|--|------|------|
| 导入时 host 识别 | bilibili.com / b23.tv | douyin.com / iesdouyin.com |
| 当前可处理 URL | 路径中可直接解析 `/video/BV...`；`b23.tv` 等短链不会自动展开 | 路径含 `/video/{aweme_id}`，或查询参数含 `modal_id` / `aweme_id` / `item_id`；分享短链不会自动展开 |
