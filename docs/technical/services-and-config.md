...
- `POST /v1/warmup` — 预加载首个已安装模型；没有模型时返回 503
...
| 可用模型 | `BAAI/bge-m3`（默认，1024 维）/ `Qwen/Qwen3-Embedding-0.6B`（1024 维） |

### API（OpenAI 兼容）

- `GET /health`
- `GET /v1/models`
- `POST /v1/warmup` — 预加载当前模型
- `POST /v1/embeddings` — JSON：`{"model","input":[...],"input_type"?}`，向量默认 L2 normalize；Qwen 的 `input_type=query` 会使用查询提示词

### Backend 如何用

- `CloudEmbeddingClient`：`POST {endpoint}/embeddings`
- 低层客户端要求 **endpoint / api_key / model 均非空**；工厂会为 loopback endpoint 自动补 `local` 占位 key，云端或局域网 endpoint 仍须配置非空 key
- Settings 指向本地服务时用 `http://<host>:8003/v1`（**需含 `/v1`**）
- 默认 yaml 常指向 Ollama（如 `http://localhost:11434/v1`）；以当前激活 preset 为准
- 对 loopback 的 :8003 endpoint，backend 会在首次非空 embedding 请求时检查 `/health`；若服务未启动且平台对应的 `services/embedding/.venv` uvicorn 存在，则在本机懒启动并最多等待 120s。其它端口、远端 endpoint 或缺少启动器时不会 spawn

应用内 Embedding 管理 API 前缀：`/api/embedding`（本地状态、安装进度、单模型或全部模型卸载）。
...
| `deploy [--asr ...] [--embedding ...]` | 修复两套服务环境，并按参数或交互选择安装缺失模型；不选模型时仅修环境 |
| `serve` | 启动 ASR:16888 + Embedding:16889，打印局域网配置提示 |
| `serve --warm` | 启动服务后调用两者的 `/v1/warmup` |

不带参数运行 `python services/node/bootstrap.py` 会进入交互菜单，可查看状态、修复环境/安装模型、卸载模型，以及冷启动或预热启动服务。
...
5. 项目运行目录的 `.env`
6. 操作系统环境变量（Pydantic nested，如 `MODELS__CHAT__ENDPOINT` 一类；以 `settings.py` 为准）
...
1. 启动 douyin_fetcher（:8002）：开发态要求其 venv 已存在；打包态使用随应用分发的可执行 sidecar
2. 若 `GET http://localhost:8000/api/health` 未就绪则 spawn backend，并最多等待 120s
3. 打包态启动 `resources/frontend/server.js` 并最多等待 30s；开发态前端由外部 Next dev server 提供，默认 `http://localhost:3000`
...
打包态的可写路径由 Electron 统一传给 backend：Windows 将 Electron 状态、业务数据和缓存分别放在 `<安装根>/data/electron`、`<安装根>/data/storage`、`<安装根>/cache`，服务运行文件复制到 `<安装根>/services`；其它平台的业务数据位于 `<userData>/data`，服务运行文件复制到 `<userData>/runtime/services`。对应项目根和数据根通过 `MEMENTO_PROJECT_ROOT`、`STORAGE__DATA_DIR` 传入。
