...
| 桌面壳 | `desktop/` | 启动 backend；开发态在 douyin_fetcher venv 存在时拉起服务，打包态启动随应用分发的 sidecar |
...
backend 可在有限条件下懒启动本地 ASR 和 Embedding：ASR endpoint 必须是 loopback 且使用 `transcriptions` 协议，Embedding endpoint 必须是 loopback 的 :8003；两者都要求对应服务的 `.venv` 启动器已存在。macOS / Linux 使用 `.venv/bin/uvicorn`，Windows 使用 `.venv/Scripts/uvicorn.exe`；其它情况须预先启动服务。
...
2. **处理**：`POST /api/videos/{id}/process`  
   - B 站默认只取软字幕；无字幕失败；可选 `?subtitle_fallback=asr` 强制走 ASR
   - 抖音主路径为下载 + ASR  
...
4. **对话**：Agent 工具  
   ...
   - 会话与消息：首条消息时前端显式建会话；支持编辑/删除 user 消息、生成中停止回填（见 [Chat 会话、编辑与停止](./chat-sessions-and-editing.md)）

更细的阶段见 [视频摄入流水线](./video-pipeline.md)；记忆三层见 [记忆系统架构](./memory-architecture.md)；Chat 交互契约见 [Chat 会话、编辑与停止](./chat-sessions-and-editing.md)。

## 运行时数据落点

默认数据根：`settings.storage.data_dir`（配置 / 默认值见代码与 yaml）。

打包态由 Electron 显式传入数据根：Windows 使用 `<安装根>/data/storage`，其它平台使用 Electron `<userData>/data`；开发态的相对路径按项目根解析。
...
- 运行时：`memento.db` 的 **active preset + app_config** 覆盖 yaml；随后是 `.env`，操作系统环境变量最后覆盖
...
- [Chat 会话、编辑与停止](./chat-sessions-and-editing.md)
