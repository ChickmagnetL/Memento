...
export function LocalModelModal({
  ...
  async function configureAndActivate(modelId: string) {
    ...
    try {
      ...
      setConfigurationMessage(
        `${SERVICE_LABELS[service]} preset configured and activated.`,
      );
      onConfigured();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Operation failed");
  ...
  async function confirmEmbeddingSwitch() {
    ...
    try {
      ...
      setPendingEmbeddingSwitch(null);
      setConfigurationMessage(
        "Embedding preset configured. Index rebuild has started and it will become active when the rebuild completes.",
      );
      onConfigured();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Operation failed");
      ...
      <div className="max-h-[80vh] w-full max-w-lg space-y-5 overflow-y-auto rounded-lg border bg-background p-5 shadow-lg">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold">
            Local {SERVICE_LABELS[service]} Models
          </h2>
          <button
            type="button"
            onClick={onClose}
              ...
              <div className="flex items-center justify-between gap-3 rounded-md border border-input bg-muted/30 p-3">
                <div className="text-sm">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">
                    {environment?.runtime_device ? "Active" : "Planned"} device: {displayedDevice.toUpperCase()}
                  </p>
                  {!environment?.venv_exists ? (
                    <p className="mt-1 text-xs text-muted-foreground">
                      First model install also creates the environment.
                    </p>
                  ) : null}
                </div>
                {environment?.venv_exists ? (
                  <Button
                    ...
                    disabled={busy}
                    onClick={() => {
                      if (window.confirm(`Remove the local ${SERVICE_LABELS[service]} environment and all its models?`)) {
              ...
              <div className="flex gap-3">
                {service === "asr" ? (
                  <label className="block flex-1 text-sm">
                    <span className="mb-1 block text-muted-foreground">Family</span>
                    <select
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between gap-3 rounded-md border border-input bg-muted/30 p-3">
                    ...
                    {selectedModel.installed ? (
                      <div className="flex gap-2">
                        ...
                        >
                          {isConfiguring ? "Configuring…" : "Configure & Activate"}
                        </Button>
                        <Button
                          type="button"
                          size="sm"
                          ...
                          disabled={busy}
                          onClick={() => {
                            if (window.confirm(`Uninstall ${selectedModel.label}?`)) {
                  ...
                  {pendingEmbeddingSwitch ? (
                    <div className="space-y-2 rounded-md border border-input bg-muted/30 p-3">
                      <p className="text-sm">
                        Activating this model requires rebuilding the embedding index.
                      </p>
                      <p className="text-xs text-muted-foreground">
                        Dimension {pendingEmbeddingSwitch.preview.current_dimension} to{" "}
                        {pendingEmbeddingSwitch.preview.new_dimension}.{" "}
                        {pendingEmbeddingSwitch.preview.indexed_document_count} indexed
