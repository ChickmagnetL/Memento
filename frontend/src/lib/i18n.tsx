...
const zhCN: Record<string, string> = {
  ...
  "Configure and enable Chat and Embedding in Settings. Chat is used to clean, summarize, and answer questions, while Embedding is used to build the index and retrieve content.": "在设置中配置并启用 Chat 和 Embedding。Chat 用于清洗、总结和回答问题，Embedding 用于建立索引和检索内容。",
  ...
  "Organize and build the index": "清洗并建立索引",
  "After processing succeeds, the document appears under Not Indexed in the Knowledge Base. We recommend clicking Clean, which cleans the transcript, generates a summary, and builds the index automatically.": "处理成功后，文档会出现在知识库的“未建立索引”中。建议点击“清洗”，系统会清洗转录文本、生成摘要并自动建立索引。",
  ...
  "Documents processed with Clean include summaries, making them better for summarization and topic exploration. Source links in answers can open the corresponding video in the desktop app.": "经过“清洗”的文档包含摘要，更适合总结和主题探索。回答中的来源链接可在桌面应用中打开对应视频。",
  "Start chatting": "开始对话",
  ...
  "When using Clean, check Chat and Embedding. When using Index, check Embedding. After fixing the configuration, return to the Knowledge Base and click Clean or Index again.": "使用“清洗”时请检查 Chat 和 Embedding；使用“建立索引”时请检查 Embedding。修复配置后返回知识库，再次点击相应操作。",
  "Chat cannot find video content": "Chat 找不到视频内容",
  "Make sure the document is under Indexed in the Knowledge Base. For a summary or overview of a video, prefer a document that has been processed with Clean.": "请确认文档位于知识库的“已建立索引”中。如需视频摘要或概览，优先使用经过“清洗”的文档。",
  ...
  "Install and manage local ASR models": "安装和管理本地 ASR 模型",
  "Install and manage local Embedding models": "安装和管理本地嵌入模型",
  "CUDA, MPS, or CPU is selected automatically.": "系统会自动选择 CUDA、MPS 或 CPU。",
  "Local ASR Models": "本地 ASR 模型",
  "Local Embedding Models": "本地嵌入模型",
  "{service} preset configured and activated.": "{service} 预设已配置并启用。",
  "Embedding preset configured. Index rebuild has started and it will become active when the rebuild completes.": "Embedding 预设已配置。索引重建已经开始，重建完成后该预设将自动启用。",
  "Environment": "运行环境",
  "Active device: {device}": "当前设备：{device}",
  "Planned device: {device}": "计划使用设备：{device}",
  "First model install also creates the environment.": "首次安装模型时也会创建运行环境。",
  "Remove the local {service} environment and all its models?": "要删除本地 {service} 运行环境及其中的全部模型吗？",
  "Select model": "选择模型",
  "Configuring…": "正在配置…",
  "Configure & Activate": "配置并启用",
  "Uninstall {model}?": "要卸载 {model} 吗？",
  "Activating this model requires rebuilding the embedding index.": "启用此模型需要重建 Embedding 索引。",
  "Confirm & Rebuild": "确认并重建",
};
